...
// Frame header and footer (SAME as LD2450)
...
void setup() {
  Serial.begin(115200);
  radarSerial.begin(256000, SERIAL_8N1, RX_PIN, TX_PIN); // Same baud rate
...
void loop() {
  ...
  while (radarSerial.available()) {
    ...
    if (bufIdx >= 4) {
      ...
      if (headerFound) {
        // RD03D: Wait for complete frame (4 header + 24 data + 2 footer = 30 bytes)
        if (bufIdx >= 30) {
          // Check footer
          if (buf[bufIdx - 2] == FRAME_FOOTER[0] && buf[bufIdx - 1] == FRAME_FOOTER[1]) {
            parseFrame(&buf[bufIdx - 30]);
            bufIdx = 0;
          }
        }
      }
...
void parseFrame(uint8_t* frame) {
  ...
  // Get target count (first byte after header)
  ...
  // RD03D can track up to 5 targets, but parse what's available
  for (int i = 0; i < 5 && i < targetCount; i++) {
    Target t;
    int offset = 1 + (i * 8);
    
    // Parse X, Y, Speed, Resolution (same as LD2450)
