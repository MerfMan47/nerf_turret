...
const uint32_t SPEED_HZ = 8000; // was 4000
const uint32_t ACCELERATION = 9000; // was 5000
...
void setup() {
  ...
  xTaskCreatePinnedToCore(
    radarProcessingTask,    // Task function
    "RadarTask",            // Name
    8192,                   // Stack size
    NULL,                   // Parameters
    2,                      // Priority (higher = more important)
    NULL,                   // Task handle
    0                       // Core 0 - radar reading and processing
  );
...
void radarProcessingTask(void* parameter) {
  ...
  while (true) {
    ...
    vTaskDelay(1 / portTICK_PERIOD_MS); // Minimal delay for responsiveness
  }
}
...
void processFrame(byte* data, size_t length) {
  ...
  // Calculate target position and update shared data
...
void unpackTarget(byte* data, Target& target) {
  ...
  target.active = !(target.x == 0 && target.y == 0 && target.speed == 0); // Is target valid?
}
...
void selectAndTrackTarget() {
  ...
  for (int i = 0; i < 3; i++) {         // NUM OF TARGETS UP TO 3
  ...
  if (closestTarget != nullptr) {
    ...
    // TODO: Add pitch calculation when you have Z-axis data or mounting height
...
void stepperControlTask(void* parameter) {
  ...
  while (true) {
    ...
    vTaskDelay(5 / portTICK_PERIOD_MS); // 5ms update rate for smooth movement
  }
}