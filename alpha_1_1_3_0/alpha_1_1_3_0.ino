...
const uint32_t ACCELERATION = 13000; // was 6000
...
const unsigned long UPDATE_INTERVAL_MS = 1000; // Update every 250ms (quarter second)
...
void selectAndTrackTarget() {
  ...
  if (closestTarget != nullptr) {
    ...
    if (abs(yawSteps - lastTargetSteps) > DEADBAND_STEPS) {
      ...
      // Target exists but change not significant enough
      lastUpdateTime = currentTime; // Still update time to maintain interval
    }
  } else {
    // No target found - update time anyway to maintain interval
