...
const int32_t DEADBAND_STEPS = 90;
...
class KalmanFilter {
private:
  float x_est;      // Estimated state (position)
  float v_est;      // Estimated velocity
  float P_pos;      // Position estimate uncertainty
  float P_vel;      // Velocity estimate uncertainty
  float Q_pos;      // Process noise for position
  float Q_vel;      // Process noise for velocity
  float R;          // Measurement noise
  float dt;         // Time step (0.1s for 10Hz radar)
  bool initialized;

public:
  // KalmanFilter() : x_est(0), v_est(0), P_pos(1000), P_vel(1000), 
  //                  Q_pos(10), Q_vel(10), R(50), dt(0.1), initialized(false) {} // origional parameters
  KalmanFilter() : x_est(0), v_est(0), P_pos(1000), P_vel(1000), 
                   Q_pos(10), Q_vel(10), R(200), dt(0.1), initialized(false) {} // experimental parameters
...
void setup() {
  ...
  Serial.print("Movement updates every ");
  Serial.print(UPDATE_INTERVAL_MS);
  Serial.println("ms with Kalman filtering");
  delay(1000);
...
void selectAndTrackTarget() {
  ...
  static unsigned long lastTargetSeenTime = 0;
  const unsigned long TARGET_TIMEOUT_MS = 500; // Target lost if not seen for 500ms
  ...
  if (closestTarget != nullptr) {
    lastTargetSeenTime = currentTime; // Update last seen time
    
    // If we switched targets, reset Kalman filters
    if (closestIndex != lastTargetIndex) {
      kalman_x.reset();
      kalman_y.reset();
      lastTargetIndex = closestIndex;
      Serial.println("Target switched - Kalman filters reset");
    }
    
    // Apply Kalman filter to smooth raw measurements
    float filtered_x = kalman_x.update(closestTarget->x);
    float filtered_y = kalman_y.update(closestTarget->y);
    
    // Calculate angles using filtered positions
    ...
    // if (abs(yawSteps - lastTargetSteps) > DEADBAND_STEPS) { // origional
    if (abs(yawSteps - lastTargetSteps) > DEADBAND_STEPS && abs(yawSteps - lastTargetSteps) < 900) { // ignores giant leeps of rotation 1200 should equal 90 degrees
      ...
      // Debug output - show raw vs filtered
      Serial.print("T");
      ...
      // Target exists but change not significant
      lastUpdateTime = currentTime;
    }
  } else {
    // NO ACTIVE TARGET FOUND
    
    // Check if target was recently lost
    if (lastTargetIndex != -1) {
      unsigned long timeSinceLastSeen = currentTime - lastTargetSeenTime;
      
      if (timeSinceLastSeen > TARGET_TIMEOUT_MS) {
        // Target truly lost - reset everything
        kalman_x.reset();
        kalman_y.reset();
        lastTargetIndex = -1;
        lastTargetSteps = 0; // CRITICAL: Reset last position
        
        Serial.print("TARGET LOST (");
        Serial.print(timeSinceLastSeen);
        Serial.println("ms) - All tracking data cleared");
      }
    }
