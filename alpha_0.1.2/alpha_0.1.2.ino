...
const uint32_t ACCELERATION = 6000;
...
const int32_t DEADBAND_STEPS = 50; // Only update if change is > 50 steps (~0.4°)

// Target filtering configuration
const float MIN_SPEED_THRESHOLD = 0.01;    // Minimum speed (cm/s) to be considered human
const int16_t MAX_SPEED_THRESHOLD = 200;  // Maximum reasonable human speed (cm/s)
const int16_t MIN_DISTANCE = 200;         // Minimum distance (mm) - ignore very close objects
const int16_t MAX_DISTANCE = 6000;        // Maximum tracking distance (mm)
...
void selectAndTrackTarget() {
  ...
  // Find closest active target that passes filters
  for (int i = 0; i < 3; i++) {
    if (radarState.targets[i].active) {
      Target* t = &radarState.targets[i];
      
      // Filter 1: Distance range check
      if (t->y < MIN_DISTANCE || t->y > MAX_DISTANCE) {
        Serial.print("Target ");
        Serial.print(i);
      ...
      // Filter 2: Speed check - filter out stationary objects (likely metal)
      int16_t absSpeed = abs(t->speed);
      if (absSpeed < MIN_SPEED_THRESHOLD) {
        Serial.print("Target ");
        Serial.print(i);
        Serial.print(" filtered: too slow (stationary metal?) speed=");
        Serial.println(absSpeed);
        continue;
      }
      
      // Filter 3: Unreasonably fast objects (likely noise)
      if (absSpeed > MAX_SPEED_THRESHOLD) {
        Serial.print("Target ");
        Serial.print(i);
        Serial.print(" filtered: too fast speed=");
        Serial.println(absSpeed);
        continue;
      }
      
      // Passed all filters - check if closest
      float distance = t->y;
  ...
  if (closestTarget != nullptr) {
    ...
    if (abs(yawSteps - lastTargetSteps) > DEADBAND_STEPS) {
      ...
      Serial.print(" speed=");
      Serial.print(closestTarget->speed);
      Serial.print(" -> Yaw: ");
    ...
    Serial.println("No valid targets (all filtered)");
  }
}
